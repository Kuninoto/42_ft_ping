...
```bash
git clone https://github.com/Kuninoto/42_ft_ping
```

3. Navigate to _42_ft_ping_
```bash
cd 42_ft_ping
```

4. Build and run (requires root privileges)
```bash
make && sudo ./ft_ping [options] <destination>
```

### Subject (v5.1): [ft_ping en_subject](./extras/en.subject_ft_ping.pdf)
